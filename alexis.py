...
def generer_voisin(tuple):
    ...
    if (position[0] - 1) in [0, 1, 2]:
        tableau1[position[0]][position[1]] = tableau1[position[0] - 1][position[1]]  # le zéro est remplacé
        tableau1[position[0] - 1][position[1]] = 0  # le zéro remplace
        LLL.append(tableau1)
    if (position[0] + 1) in [0, 1, 2]:
        tableau2[position[0]][position[1]] = tableau1[position[0] + 1][position[1]]  # le zéro est remplacé
        tableau2[position[0] + 1][position[1]] = 0  # le zéro remplace
        LLL.append(tableau2)
    if (position[1] - 1) in [0, 1, 2]:
        tableau3[position[0]][position[1]] = tableau1[position[0]][position[1] - 1]  # le zéro est remplacé
        tableau3[position[0]][position[1] - 1] = 0  # le zéro remplace
        LLL.append(tableau3)
    if (position[0] + 1) in [0, 1, 2]:
        tableau4[position[0]][position[1]] = tableau1[position[0]][position[1] + 1]  # le zéro est remplacé
